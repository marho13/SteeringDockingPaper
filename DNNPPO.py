...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, has_continuous_action_space, action_std_init):
        ...
        self.criticIn = nn.Linear(state_dim, 64)
        self.hiddenCritic = nn.Linear(64, 64)
    ...
    def getCritic(self):
        return list(self.criticIn.parameters()) +  list(self.hiddenCritic.parameters()) + list(self.criticout.parameters())
    ...
    def critic(self, state):
        print(state, len(state))
        x = self.criticIn(state)
        x = self.hiddenCritic(x)
        return self.criticout(x)
...
# Create the class which includes the old policy and the new policy for updating
class PPO:
    def __init__(self, state_dim, action_dim, lr_actor, lr_critic, gamma, K_epochs, eps_clip,
        ...
        self.optimizer = torch.optim.Adam([{'params':self.policy.getActor(), 'lr':lr_actor},{'params': self.policy.getCritic(), 'lr':lr_actor}])
    ...
    def save(self, checkpoint):
        torch.save(self.policy_old.state_dict(), checkpoint)

    def load(self, checkpoint):
        self.policy_old.load_state_dict(torch.load(checkpoint, map_location=lambda storage, loc:storage))
        self.policy.load_state_dict(torch.load(checkpoint, map_location=lambda storage, loc: storage))